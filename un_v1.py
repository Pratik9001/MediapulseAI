...
TMDB_API_KEY = "5bf4c97af040aeb12d79e32757777f87" 
    ...
    with rec_tab:
        ...
        if not active_recs.empty:
            ...
            if rec_type == "Collaborative Filtering":
                # Get the movies the selected user has already watched/reviewed
                user_history = reviews_df[reviews_df['username'] == selected_user]['movie'].unique()
                display_history = user_history[:10]
                history_str = ", ".join(display_history)
                if len(user_history) > 10:
                    history_str += ", ..."
                
                st.subheader(f"👤 Profile: {selected_user}")
                st.markdown(f"**User Watched:** {history_str}")
            ...
            for i, (_, row) in enumerate(active_recs.iterrows()):
                with cols[i]:
                    st.image(get_safe_poster(row['poster_url'], row['title']), use_container_width=True)
...
with tab3:
    ...
    if analyze_btn or 'sa_results' in st.session_state:
        # --- NEW: Fetch and Display Movie Metadata ---
        movie_info = df[df['title'] == movie_for_sa].iloc[0]
        
        with st.container():
            col_img, col_txt = st.columns([1, 4])
            with col_img:
                st.image(get_safe_poster(movie_info['poster_url'], movie_for_sa), width=150)
            with col_txt:
                st.subheader(f"{movie_info['title']} ({movie_info['year']})")
                st.write(f"**Director:** {', '.join(movie_info['directors']) if isinstance(movie_info['directors'], list) else 'Unknown'}")
                st.write(f"**Genres:** {', '.join(movie_info['genres']) if isinstance(movie_info['genres'], list) else 'Unknown'}")
                st.write(f"**Rating:** ⭐ {movie_info['rating_val']}" if not pd.isna(movie_info['rating_val']) else "Rating: N/A")
                st.write(f"**Synopsis:** {movie_info['synopsis']}" if pd.notna(movie_info['synopsis']) else "Synopsis: N/A")
        st.markdown("---")
        ...
        if movie_reviews.empty:
            ...
            with st.spinner("Running NLP sentiment models..."):
                ...
                overall_mood = "😊" if avg_polarity > 0.1 else " 😠" if avg_polarity < -0.1 else "😐"
